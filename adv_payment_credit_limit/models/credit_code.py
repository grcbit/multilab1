        ...
        if account_invoice_search:
            for account_invoice in account_invoice_search:
                if account_invoice.invoice_payment_term_id:
                    totlines = account_invoice.invoice_payment_term_id.with_context(currency_id=account_invoice.currency_id.id).compute(account_invoice.amount_total, account_invoice.invoice_date)
                    for total_line in totlines:
                        open_amount = total_line[1]
                        if not account_invoice.currency_id == partner_currency_id:
                            open_amount = account_invoice.currency_id.compute(total_line[1], partner_currency_id)
                        date = datetime.strptime(total_line[0], "%Y-%m-%d").date()
                        if check_date >= date:
                            if account_invoice.move_type == 'out_invoice':
                                past_due_amt += open_amount
                            elif account_invoice.move_type == 'out_refund':
                                past_due_amt -= open_amount
                else:
                ...
                for account_invoice in account_invoice_search:
                    if account_invoice.invoice_payment_term_id:
                        ...
                        for total_line in totlines:
                            ...
                            elif account_invoice.move_type == 'out_invoice':
                                past_due_amt += total_line[1]
                            elif account_invoice.move_type == 'out_refund':
                                past_due_amt -= total_line[1]
                    else:
...
class AccountInvoice(models.Model):
    ...
    def _onchange_payment_term_date_invoice(self):
        ...
        if not self.invoice_payment_term_id:
            ...
            payment_term_list = payment_term.with_context(currency_id=self.company_id.currency_id.id).compute(value=1, date_ref=invoice_date)
            datetime_list = []
            for payment_term in payment_term_list:
                datetime_list.append(payment_term[0])
            self.invoice_date_due = max(datetime_list)
